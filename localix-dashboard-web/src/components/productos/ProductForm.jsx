...
const MAX_IMAGE_SIZE = 1 * 1024 * 1024; // 1MB - Reducido para evitar error 413
...
export default function ProductForm({ productToEdit, onSuccess, onCancel, compact = false }) {
  ...
  const ensureGeneralCategory = async () => {
    try {
      ...
      if (!window.electronAPI) {
        console.warn('⚠️ electronAPI no disponible, usando categorías por defecto');
        return null;
      }
      ...
      // Paso 2: Intentar crear la categoría "General"
      try {
        ...
        // Paso 3: Como último recurso, buscar cualquier categoría que contenga "general"
        try {
          const categoriesData = await window.electronAPI.categorias.listar();
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const categoriesData = await window.electronAPI.categorias.listar();
          ...
          console.warn('⚠️ ventasAPI no disponible para cargar categorías');
          setCategories([]);
        }
        ...
        product = await window.electronAPI.productos.obtener(validSlug);
          }
          if (product) {
          reset({
            ...product,
          ...
          if (product.imagen_principal_url) {
            ...
            console.warn('⚠️ No se pudo cargar el producto:', validSlug);
    ...
    // Configurar listener de errores solo si electronAPI está disponible
    let cleanupErrorListener = null;
    if (window.electronAPI && window.electronAPI.eventos && window.electronAPI.eventos.onError) {
      cleanupErrorListener = window.electronAPI.eventos.onError((err) => {
    ...
    if (file) {
      ...
      try {
        ...
        if (processedFile.size > MAX_IMAGE_SIZE) {
          setError(`La imagen sigue siendo muy grande después de la compresión. Tamaño máximo: ${(MAX_IMAGE_SIZE / (1024 * 1024)).toFixed(1)}MB`);
          return;
        }
...
const onSubmit = async (data) => {
  ...
  try {
    ...
    if (validSlug) {
      ...
      // Preparar payload con imagen como File si existe
      let payload = { ...data };
      console.log('🔍 DEBUG - data.imagen_principal:', data.imagen_principal);
      ...
      if (data.imagen_principal) {
        // Si es un objeto con data (desde el input file), extraer el File original
        if (data.imagen_principal.data instanceof File) {
          console.log('✅ Imagen detectada como objeto con File data');
          payload.imagen_principal = data.imagen_principal.data;
        } else if (data.imagen_principal instanceof File) {
          console.log('✅ Imagen detectada como File directo');
          // Si ya es un File, mantenerlo
          payload.imagen_principal = data.imagen_principal;
        } else if (typeof data.imagen_principal.data === 'string' && data.imagen_principal.name && data.imagen_principal.type) {
          console.log('🔄 Convirtiendo base64 a File object');
          // Convertir base64 a File
          const fileFromBase64 = base64ToFile(
            data.imagen_principal.data,
            data.imagen_principal.name,
            data.imagen_principal.type
          );
          if (fileFromBase64) {
            console.log('✅ Conversión exitosa de base64 a File');
            payload.imagen_principal = fileFromBase64;
          } else {
            console.log('❌ Error en conversión base64 a File, removiendo del payload');
            delete payload.imagen_principal;
          }
        } else {
          console.log('❌ Imagen no es un File válido, removiendo del payload');
          // Si no es un File válido, remover la imagen del payload
          delete payload.imagen_principal;
        }
      } else {
        console.log('ℹ️ No hay imagen_principal en data');
      }
      
      console.log('📤 Payload final para actualización:', {
        ...payload,
        imagen_principal: payload.imagen_principal ? {
          name: payload.imagen_principal.name,
          type: payload.imagen_principal.type,
          size: payload.imagen_principal.size,
          isFile: payload.imagen_principal instanceof File
        } : 'NO_IMAGE'
      });
      if (window.electronAPI) {
          productoResult = await window.electronAPI.productos.actualizar(validSlug, payload);
      } else {
          console.warn('⚠️ electronAPI no disponible para actualizar producto');
          setError('API no disponible para actualizar el producto');
        return;
      }
      ...
      // Preparar payload con imagen como File si existe
      let payload = { ...data };
      console.log('🔍 DEBUG CREACIÓN - data.imagen_principal:', data.imagen_principal);
      
      // Función para convertir base64 a File (reutilizada)
      const base64ToFile = (base64String, filename, mimeType) => {
        try {
          const byteCharacters = atob(base64String);
          const byteNumbers = new Array(byteCharacters.length);
      ...
      if (data.imagen_principal) {
        // Si es un objeto con data (desde el input file), extraer el File original
        if (data.imagen_principal.data instanceof File) {
          console.log('✅ CREACIÓN - Imagen detectada como objeto con File data');
          payload.imagen_principal = data.imagen_principal.data;
        } else if (data.imagen_principal instanceof File) {
          console.log('✅ CREACIÓN - Imagen detectada como File directo');
          // Si ya es un File, mantenerlo
          payload.imagen_principal = data.imagen_principal;
        } else if (typeof data.imagen_principal.data === 'string' && data.imagen_principal.name && data.imagen_principal.type) {
          console.log('🔄 CREACIÓN - Convirtiendo base64 a File object');
          // Convertir base64 a File
          const fileFromBase64 = base64ToFile(
            data.imagen_principal.data,
            data.imagen_principal.name,
          ...
          if (fileFromBase64) {
            console.log('✅ CREACIÓN - Conversión exitosa de base64 a File');
            payload.imagen_principal = fileFromBase64;
          } else {
            console.log('❌ CREACIÓN - Error en conversión base64 a File, removiendo del payload');
            delete payload.imagen_principal;
          }
        } else {
          console.log('❌ CREACIÓN - Imagen no es un File válido, removiendo del payload');
          // Si no es un File válido, remover la imagen del payload
          delete payload.imagen_principal;
        }
      } else {
        console.log('ℹ️ CREACIÓN - No hay imagen_principal en data');
      }
      
      console.log('📤 CREACIÓN - Payload final:', {
        ...payload,
        imagen_principal: payload.imagen_principal ? {
          name: payload.imagen_principal.name,
          type: payload.imagen_principal.type,
      ...
      if (window.electronAPI) {
          ...
          console.warn('⚠️ electronAPI no disponible para crear producto');
          setError('API no disponible para crear el producto');
        return;
      }
