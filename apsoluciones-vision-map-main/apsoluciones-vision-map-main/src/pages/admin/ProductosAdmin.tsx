...
import { apiService, Producto, ApiResponse } from '@/services/api';
...
const ProductosAdmin: React.FC = () => {
  ...
  const loadProductos = async (page: number = 1, search: string = '') => {
    try {
      setLoading(true);
      setError(null);
      
      const response: ApiResponse<Producto> = await apiService.getProductos({
        page,
