...
import { apiService, Categoria, ApiResponse } from '@/services/api';
...
const CategoriasAdmin: React.FC = () => {
  ...
  const loadCategorias = async () => {
    try {
      ...
      const response: ApiResponse<Categoria> = await apiService.getCategorias();
      setCategorias(response.results);
    } catch (err) {
      console.error('Error loading categorias:', err);
      setError('Error al cargar las categorías. Verifica que el backend esté funcionando.');
    } finally {
      setLoading(false);
    }
  };
